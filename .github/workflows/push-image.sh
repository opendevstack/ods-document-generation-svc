...
# This script will push a previously build ods-document-generation-svc:local image with the corresponding tag to dockerhub.
...
if [[ $DOCKERTAG != 'none' ]]; then
  echo "Pushing docker image opendevstackorg/ods-document-generation-svc:$DOCKERTAG"

  echo "$PASSWORD" | docker login -u "$USER" --password-stdin
  docker tag ods-document-generation-svc:local opendevstackorg/ods-document-generation-svc:$DOCKERTAG
  docker push opendevstackorg/ods-document-generation-svc:$DOCKERTAG
  docker logout
