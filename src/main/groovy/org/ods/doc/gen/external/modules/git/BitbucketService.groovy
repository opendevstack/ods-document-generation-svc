...
class BitbucketService {
    ...
    static String userTokenSecretYml(String tokenSecretName, String username, String password) {
        """\
          apiVersion: v1
          data:
            username: ${org.ods.doc.gen.external.modules.jenkins.AuthUtil.base64(username)}
            password: ${org.ods.doc.gen.external.modules.jenkins.AuthUtil.base64(password)}
          kind: Secret
...
repos/${repo}/commits/${gitCommit}/reports/${data.key}"""
    ...
    Map<String, String> createUserToken() {
        ...
        def res = ''
        def payload = """{"name": "ods-jenkins-external.modulesrary-${openShiftCdProject}", "permissions": ["PROJECT_WRITE", "REPO_WRITE"]}"""
        script.withCredentials(
            [script.usernamePassword(
            ...
            res = script.sh(
                ...
                script: """set +x; curl \\
                  ...
                  --request PUT \\
                  --header \"Content-Type: application/json\" \\
                  --header \"${org.ods.doc.gen.external.modules.jenkins.AuthUtil.header(org.ods.doc.gen.external.modules.jenkins.AuthUtil.SCHEME_BASIC, username, password)}\" \\
                  --data '${payload}' \\
                  ${url}
                """
            ).trim()
