...
class MROPipelineUtilSpec extends SpecHelper {
    ...
    def "load a repo's pipeline config"() {
        ...
        def pipelineConfigFile = Paths.get(repoPath, PipelineConfig.FILE_NAMES.first())
    ...
    def "load a repo's pipeline config with invalid repo"() {
        ...
        def componentMetadataFile = Paths.get(repoPath, PipelineConfig.COMPONENT_METADATA_FILE_NAME)
        def pipelineConfigFile = Paths.get(repoPath, PipelineConfig.FILE_NAMES.first())
        ...
        then:
        e = thrown(IllegalArgumentException)
        e.message == "Error: unable to parse pipeline phase config. Attribute 'phase.type' contains an unsupported value 'someType' in phase 'build'. Supported types are: ${PipelineConfig.PHASE_EXECUTOR_TYPES}."
    ...
    def "load multiple repos' pipeline configs"() {
        ...
        repos.each { repo ->
            ...
            pipelineConfigFileMap[repo.id] = Paths.get(repoPath, PipelineConfig.FILE_NAMES.first())
        }
